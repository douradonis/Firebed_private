...
# add vendor path for vendorized dependencies (eg. vendor/mydatanaut)
...
mydatanaut_pkg = None
try:
    import mydatanaut as mydatanaut_pkg  # vendor/mydatanaut package
    HAS_MYDATANAUT = True
except Exception:
    HAS_MYDATANAUT = False
...
def fetch_docs_with_mydatanaut(df, dt, vat, aade_user, aade_key):
    if not HAS_MYDATANAUT:
        return None
    try:
        # Attempt common client patterns (you might need to adapt if mydatanaut api differs)
        client = None
        try:
...
def fetch():
    ...
    if request.method == "POST":
        ...
        aade_user = AADE_USER_ENV; aade_key = AADE_KEY_ENV
        if selected:
            c = next((x for x in creds if x.get("name")==selected), None)
            if c:
                aade_user = c.get("user",""); aade_key = c.get("key","")
...
def cron_fetch():
    ...
    creds = load_credentials()
    if creds:
        aade_user = creds[0].get("user",""); aade_key = creds[0].get("key",""); vat = creds[0].get("vat","")
    else:
        aade_user = AADE_USER_ENV; aade_key = AADE_KEY_ENV; vat = os.getenv("ENTITY_VAT","")
